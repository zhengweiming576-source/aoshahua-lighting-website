...
--
-- The owner pasted a Brevo key that had been soft-wrapped, so the stored secret
-- contained an embedded line break. A newline inside an HTTP header value makes
-- the whole request malformed, and Brevo's gateway answers with the very
-- misleading "Input must be a valid JSON object" instead of anything about the
-- key. Strip every whitespace character when reading the secret so a wrapped
-- paste can never break sending again.
--
-- Applied: 2026-09-10
